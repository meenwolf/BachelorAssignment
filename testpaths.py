...
for building in os.listdir(PATH):
    ...
    paths, attributes = svg2paths(PATH + f"\\{building}")
    lengthForOneMeter= 1
    ...
    for path, attr in zip(paths, attributes):
        for i, line in enumerate(path):
            ...
            if not start in coordinateToNode:
                coordinateToNode[start] = nextnode
                nodeToCoordinate[nextnode] = {'Location': start}
                snode = nextnode
                nextnode += 1
            else:
                snode = coordinateToNode[start]

            if not end in coordinateToNode:
                coordinateToNode[end] = nextnode
                nodeToCoordinate[nextnode] = {'Location': end}
                enode = nextnode
                nextnode += 1
            else:
                enode = coordinateToNode[end]
...
print(f"are all vertices reachable from vertex 1 before adding dummy vertex?? {len(getReachable(neighbours, 1))} are reachable while vdum is: {vdum}")
connectedvertices=set()
components=[]
for i in range(vdum):
    if i not in connectedvertices:
...
for i, component in enumerate(components):
    ...
    for vertex in component:
        if vertex in nodeToCoordinate:
            building, floor = getBuildingFloor(nodeToCoordinate, vertex)
            if building in buildings:
                buildings[building].update(floor)
            else:
                buildings[building] = {floor}
        else:
            print(f"vertex {vertex} is not in nodeToCoordinate so must be an elevator vertex")
            found=False
            for key, value in elevatorVertices.items():
                if value == vertex:
                    found=key
            if found:
                print(f"vertex {vertex} is elevator {found}")
            else:
...
#
# model, varshall, varsdegree = runModel(hallways, vdum, maxtime=50, printtime= 15, logfile= "\\log0601try2.log")
